...
import base.android.compose.LocalNavigation
    ...
    private fun MainContent() {
        ...
        CompositionLocalProvider(LocalNavigation provides navController) {
            NavHost(navController, startDestination = SCREEN_SPLASH) {
                composable(SCREEN_SPLASH) { SplashScreen(navController) }
                composable(SCREEN_DASHBOARD) { DashboardScreen(navController) }
                composable(
                    "$SCREEN_USER/{userId}",
